...
class PickerView @JvmOverloads constructor(
    ...
    private fun draw(canvas: Canvas, type: Int, position: Int, paint: Paint) {
        ...
        if ((baseline + fontMetrics.ascent) > 0 && (baseline + fontMetrics.descent) < height) {
            canvas.drawText(
                mItems[mSelectPosition + type * position],
                x - textWidth / 2 + (paddingStart - paddingEnd),
                baseline,
                paint
            )
        }
        if (position == 0) {
            ...
            if (!TextUtils.isEmpty(mText)) {
                val startX =
                    mPaintSelect.measureText(getMaxLengthItem()) / 2 + x - textWidth / 2 + TEXT_MARGIN + (paddingStart - paddingEnd)
                val anInt = mPaintText.fontMetricsInt
                canvas.drawText(
                    mText!!,
                    startX,
                    height / 2f + (anInt.bottom - anInt.top) / 2f - anInt.descent,
                    mPaintText
                )
            }
            val line =
                height / 2f + (fontMetrics.bottom - fontMetrics.top) / 2f - fontMetrics.descent
            canvas.drawLine(
                0f,
                line + fontMetrics.ascent - 2.dp2px(),
                width.toFloat(),
                line + fontMetrics.ascent - 2.dp2px(),
                mPaintLine
            )
            canvas.drawLine(
                0f,
                line + fontMetrics.descent + 2.dp2px(),
                width.toFloat(),
                line + fontMetrics.descent + 2.dp2px(),
                mPaintLine
            )
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (abs(pickerView.mMoveDistance) < SPEED) {
                ...
                if (pickerView.mTask != null) {
                    ...
                    pickerView.mOnSelect?.invoke(
                        pickerView,
                        pickerView.mItems[pickerView.mSelectPosition]
                    )
                }
            } else {
                pickerView.mMoveDistance -= pickerView.mMoveDistance / abs(pickerView.mMoveDistance) * SPEED
            }
